...
import { MapPin, Mail, Lock, ArrowRight, Loader2, Home, Sun, Moon, Navigation } from 'lucide-react';
import { motion } from 'motion/react';

export default function Login({ onLogin, theme, setTheme }: { onLogin: (user: any, token: string) => void, theme: 'light' | 'dark', setTheme: (t: 'light' | 'dark') => void }) {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-6 relative overflow-hidden transition-colors duration-500 ${theme === 'dark' ? 'bg-[#0a0a0a]' : 'bg-[#f8fafc]'}`}>
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] blur-[120px] rounded-full ${theme === 'dark' ? 'bg-blue-600/10' : 'bg-blue-600/5'}`}></div>
      </div>

      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className={`w-full max-w-md backdrop-blur-xl border p-8 lg:p-10 rounded-[2.5rem] shadow-2xl relative z-10 ${theme === 'dark' ? 'bg-[#111]/80 border-white/5' : 'bg-white/80 border-black/5'}`}
      >
        ...
        <div className="mt-10 pt-8 border-t border-slate-500/10">
          <p className="text-center text-slate-500 text-sm font-medium">
            New operative? {' '}
            <Link to="/signup" className="text-blue-500 font-black hover:text-blue-400 transition-colors uppercase tracking-widest text-xs">Create credentials</Link>
          </p>
        </div>
      </motion.div>
    </div>
